  ...
  return (
    <div className="w-full max-w-7xl mx-auto py-16 px-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 lg:grid-rows-2 gap-4 mb-4">
        {/* Article 1: Massive Hero (Adjusted to col-span-7) */}
        {articles[0] && (
          <a href={`/article/${articles[0].slug}`} className="lg:col-span-7 lg:row-span-2 relative border border-white/10 bg-black overflow-hidden group min-h-[400px] flex flex-col justify-end p-8 hover:border-axim-purple/50 transition-colors">
            {articles[0]._embedded?.['wp:featuredmedia']?.[0]?.source_url && (
               <img src={articles[0]._embedded['wp:featuredmedia'][0].source_url} alt="" className="absolute inset-0 w-full h-full object-cover opacity-30 group-hover:opacity-50 group-hover:scale-105 transition-all duration-700" />
            )}
            {/* Thematic Purple Overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A] via-axim-purple/20 to-transparent z-0 mix-blend-multiply" />
        ...
        {/* Article 2: Medium Top Right (Adjusted to col-span-5) */}
        {articles[1] && (
          <a href={`/article/${articles[1].slug}`} className="lg:col-span-5 lg:row-span-1 relative border border-white/10 bg-black overflow-hidden group min-h-[220px] flex flex-col justify-end p-6 hover:border-axim-purple/50 transition-colors">
            ...
            <div className="absolute inset-0 bg-axim-purple/10 mix-blend-overlay z-0" />
            ...
            <div className="relative z-10">
              ...
              <div className="text-xs text-zinc-400 line-clamp-2" dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(articles[1].excerpt?.rendered || '')}} />
            </div>
          </a>
        )}

        {/* Article 3: Medium Bottom Right (Adjusted to col-span-5) */}
        {articles[2] && (
           <a href={`/article/${articles[2].slug}`} className="lg:col-span-5 lg:row-span-1 relative border border-white/10 bg-black overflow-hidden group min-h-[220px] flex flex-col justify-end p-6 hover:border-axim-purple/50 transition-colors">
            ...
            <div className="relative z-10">
              ...
              <div className="text-xs text-zinc-400 line-clamp-2" dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(articles[2].excerpt?.rendered || '')}} />
            </div>
          </a>
        )}
      </div>

      {/* Fibonacci Bottom Section: 3 Small Articles with Thematic Backgrounds */}
      {articles.length > 3 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {articles.slice(3, 6).map(article => (
            <a key={article.id} href={`/article/${article.slug}`} className="relative block border border-white/10 bg-black overflow-hidden group hover:border-axim-purple/50 transition-colors flex flex-col justify-end p-6 min-h-[160px]">
              {article._embedded?.['wp:featuredmedia']?.[0]?.source_url && (
                 <img src={article._embedded['wp:featuredmedia'][0].source_url} alt="" className="absolute inset-0 w-full h-full object-cover opacity-10 group-hover:opacity-30 transition-all duration-700" />
              )}
              <div className="absolute inset-0 bg-axim-purple/5 mix-blend-overlay z-0" />
