...
export default function SystemBreadcrumb() {
  ...
  return (
    ...
    <div className="w-full bg-black/60 backdrop-blur-md border-b border-white/10 px-6 md:px-12 py-2 flex items-center gap-2 font-mono text-[0.6rem] uppercase tracking-widest text-zinc-500 z-40 relative mt-20 min-h-[36px] overflow-hidden">
      {breadcrumbs.map((crumb, index) => {
        const isLast = index === breadcrumbs.length - 1;
        return (
          <React.Fragment key={`${crumb.path}-${index}`}>
            {isLast ? (
              <span className="text-axim-purple drop-shadow-[0_0_8px_#7D00FF] animate-fade-in" aria-current="page">{crumb.name}</span>
            ) : (
              <>
                <Link to={crumb.path} className="hover:text-white transition-colors group flex items-center gap-2" onClick={() => {
                  logTelemetry('breadcrumb_clicked', { path: crumb.path, label: crumb.name });
                }}>{crumb.name}</Link>
                <SafeIcon icon={LuChevronRight} className="w-3 h-3 text-zinc-700 transition-transform duration-300 group-hover:translate-x-1" />
              </>
            )}
          </React.Fragment>
        );
